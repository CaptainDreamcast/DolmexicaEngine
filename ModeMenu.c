...
sprintf(FileDir, "%s/MENU/MAINMENU.ogg", FileSystemUsed);
SoundLoad(FileDir);
...
else *ModeBackGroundAlternateTexture = NULL;
...
FileRead(InputFile, ModeMenuData, sizeof(ModeMenuStruct));
FileClose(InputFile);

if(ModeMenuData->Magic != 'D'){
ModeMenuData->AlternateColor = RedColor;
ModeMenuData->AlternateColorMinValue = 0.0f;
ModeMenuData->AlternateColorMaxValue = 1.0f;
}

fs_romdisk_unmount(MountPoint);

sprintf(FileDir, "%s/MENU/MAINMENU.ogg", FileSystemUsed);
SoundPlay(FileDir, ModeMenuTrack);
...
void DrawModeMenuBackGround(pvr_ptr_t Texture){
    ...
    vert.z = 1;
    vert.u = 1.0;
    vert.v = 1.0;    vert.flags = PVR_CMD_VERTEX_EOL;
    pvr_prim(&vert, sizeof(vert));


}

void DrawModeBackGround(int WhichMode, pvr_ptr_t ModeBackGroundTexture, pvr_ptr_t ModeBackGroundAlternateTexture, int ScreenPositionX, int ScreenPositionY, int WhichModeSelected, int Ticks, char SelectionDone, uint8 AlternateColor, float AlternateColorMinValue, float AlternateColorMaxValue){ 
...
float AlternateTextureTransparency, Red, Green, Blue;
...
if(ModeBackGroundAlternateTexture != NULL){
...
AlternateTextureTransparency = ((AlternateColorMaxValue-AlternateColorMinValue)*(1.0f-AlternateTextureTransparency))+AlternateColorMinValue;

CheckColor(AlternateColor, Red, Green, Blue, 1.0f, AlternateTextureTransparency);
...
while(TimeToGoOn == 0){
 ...
 DrawModeBackGround(DirtyDeedsDoneDirtCheap+1, ModeBackGroundTexture, ModeBackGroundAlternateTexture, ModeMenuData.SingleMode[DirtyDeedsDoneDirtCheap].PositionX, ModeMenuData.SingleMode[DirtyDeedsDoneDirtCheap].PositionY, WhichModeSelected, Ticks, SelectionDone, ModeMenuData.AlternateColor, ModeMenuData.AlternateColorMinValue, ModeMenuData.AlternateColorMaxValue);
...
pvr_mem_free(ModeMenuBackGroundTexture);
pvr_mem_free(ModeBackGroundTexture);
if(ModeBackGroundAlternateTexture != NULL) pvr_mem_free(ModeBackGroundAlternateTexture);
