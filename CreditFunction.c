...
BailOutValue = FileTotal(*InputFile);
OriginalOffset = FileTell(*InputFile);
ZeroValue = 0;

LineLength = 0;
do{
FileRead((*InputFile), &ZeroValue, 1);
LineLength++;
...
FileSeek((*InputFile), OriginalOffset, 0);
FileRead((*InputFile), SingleLineData[DirtyDeedsDoneDirtCheap].Text, LineLength);
...
(*InputFile) = FileOpen(OriginalFileDir, O_RDONLY);
FileSeek((*InputFile), 0, 0);
FileRead((*InputFile), CreditHeaderData, sizeof(CreditHeaderStruct));
FileClose(*InputFile);
