...
uint8 AlternateColor;
uint8 Separator1[3];

float AlternateColorMinValue;
float AlternateColorMaxValue;
...
char RiffStuff[8];
char WaveIdentifier[4];
char FormatIdentifier[4]; //0x10
...
WaveHeaderStruct WaveHeaderData;
uint32 BufferSize;
int Counter;
...
char Array[SingleOptionMenuArraySize];
...
uint8 Version;
uint8 Magic;
uint8 HoldingSpeedThreshold;
uint8 BackGroundColor;

short TitlePositionX;
short TitlePositionY;

short TitlePositionZ;
uint16 TitleFontSize;

uint8 SelectedOptionColor;
uint8 NonSelectedOptionColor;
uint8 Separator1[2];

char TitleArray[SingleOptionMenuArraySize];

} OptionMenuHeaderStruct;


typedef struct{

OptionMenuHeaderStruct OptionMenuHeaderData;
SingleOptionMenuStruct SingleOptionMenuData[OptionMenuOptionAmount*2];
...
char FirstTimerStartValue;
char SecondTimerStartValue;

} ModifiableOptionStruct;