int SurvivalMode(int* WhichPlayerActive, ModifiableOptionStruct* ModifiableOptionData){
...
SurvivalStage = ActualFightStage;
 ...
 if(SurvivalStage == ActualFightStage){
  Winner = -1;
  Winner = NewmalFight(SurvivalModeIdentifier, CharacterName, BackGroundName, CharIsHuman, CharHealth, PlayerID, EnemiesBeaten, &Difficulty, ModifiableOptionData->FirstTimerStartValue, ModifiableOptionData->SecondTimerStartValue);
  //Winner = CharacterOne;
  printf("Winner: (%d)\n", Winner);
