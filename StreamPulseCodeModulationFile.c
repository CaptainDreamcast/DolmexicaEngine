...
uint8 StreamingBuffer[SND_STREAM_BUFFER_MAX];
...
void SeparateChannels(void *SampleBuffer, int BufferSize, int Stereo) {
...
void FillStreamingBuffer(snd_stream_hnd_t StreamingHandler) {
void *SampleBuffer;
int SamplesGotten;
...
if (streams[StreamingHandler].stereo) SampleBuffer = streams[StreamingHandler].get_data(StreamingHandler, streams[StreamingHandler].buffer_size, &SamplesGotten);
else SampleBuffer = streams[StreamingHandler].get_data(StreamingHandler, streams[StreamingHandler].buffer_size/2, &SamplesGotten);

//process_filters(StreamingHandler, &SampleBuffer, &SamplesGotten);
SeparateChannels(SampleBuffer, (streams[StreamingHandler].buffer_size/2), streams[StreamingHandler].stereo);
spu_memload(streams[StreamingHandler].spu_ram_sch[0] + (streams[StreamingHandler].buffer_size/2)*0, (uint8*)StereoSeparationBuffer[0], streams[StreamingHandler].buffer_size/2);
spu_memload(streams[StreamingHandler].spu_ram_sch[1] + (streams[StreamingHandler].buffer_size/2)*0, (uint8*)StereoSeparationBuffer[1], streams[StreamingHandler].buffer_size/2);
...
spu_memload(streams[StreamingHandler].spu_ram_sch[0] + (streams[StreamingHandler].buffer_size/2)*1, (uint8*)StereoSeparationBuffer[0], streams[StreamingHandler].buffer_size/2);
spu_memload(streams[StreamingHandler].spu_ram_sch[1] + (streams[StreamingHandler].buffer_size/2)*1, (uint8*)StereoSeparationBuffer[1], streams[StreamingHandler].buffer_size/2);


streams[StreamingHandler].last_write_pos = 0;
streams[StreamingHandler].curbuffer = 0;
}


void StartStreaming(snd_stream_hnd_t StreamingHandler, StreamingStruct StreamingData){
...
streams[StreamingHandler].stereo = StreamingData.WaveHeaderData.Channels-1;
streams[StreamingHandler].frequency = StreamingData.WaveHeaderData.Frequency;

printf("Stereo: (%d)\n", streams[StreamingHandler].stereo);
printf("Frequency: (%d)\n", streams[StreamingHandler].frequency);
printf("Format: (%d)\n", StreamingData.WaveHeaderData.Format);

snd_sh4_to_aica_stop();
FillStreamingBuffer(StreamingHandler);

if(StreamingData.WaveHeaderData.Format == FormatADPCM) chan->type = AICA_SM_ADPCM;
else chan->type = AICA_SM_16BIT;
...
chan->base = streams[StreamingHandler].spu_ram_sch[0];
chan->length = (SND_STREAM_BUFFER_MAX/2);
chan->loop = 1;
chan->loopstart = 0;
chan->loopend = (SND_STREAM_BUFFER_MAX/2);
chan->freq = streams[StreamingHandler].frequency;
chan->vol = (VolumeCDDA*255)/15;
chan->pan = 0;
snd_sh4_to_aica(tmp, cmd->size);

cmd->cmd_id = streams[StreamingHandler].ch[1];
chan->base = streams[StreamingHandler].spu_ram_sch[1];
chan->pan = 255;
snd_sh4_to_aica(tmp, cmd->size);

cmd->cmd_id = ((1 << streams[StreamingHandler].ch[0]) | (1 << streams[StreamingHandler].ch[1]));
...
int CheckForSamples(snd_stream_hnd_t StreamingHandler, StreamingStruct StreamingData){
...
CHECK_HND(StreamingHandler);
if (!streams[StreamingHandler].get_data) return -1;

SampleBuffer = NULL;

PositionFirstChannel = g2_read_32(SPU_RAM_BASE + AICA_CHANNEL(streams[StreamingHandler].ch[0]) + offsetof(aica_channel_t, pos));
PositionSecondChannel = g2_read_32(SPU_RAM_BASE + AICA_CHANNEL(streams[StreamingHandler].ch[1]) + offsetof(aica_channel_t, pos));

if(StreamingData.WaveHeaderData.Format == FormatADPCM){
SamplesInBuffer = streams[StreamingHandler].buffer_size*2;
LastWritePositionBytes = streams[StreamingHandler].last_write_pos/2;
}
else{
SamplesInBuffer = streams[StreamingHandler].buffer_size/2;
LastWritePositionBytes = streams[StreamingHandler].last_write_pos*2;
...
printf("snd_stream_poll: chan0(%d).pos = %ld (%08lx)\n", streams[StreamingHandler].ch[0], PositionFirstChannel, PositionFirstChannel);
return -1;
}
	
CurrentBufferPosition = (PositionFirstChannel < PositionSecondChannel)?(PositionFirstChannel):(PositionSecondChannel);

if(streams[StreamingHandler].last_write_pos <= CurrentBufferPosition) SamplesNeeded = CurrentBufferPosition - streams[StreamingHandler].last_write_pos;
else SamplesNeeded = SamplesInBuffer - streams[StreamingHandler].last_write_pos;
SamplesNeeded &= ~0x7ff;

if(StreamingData.WaveHeaderData.Format == FormatADPCM) SizeNeeded = SamplesNeeded/2;
else SizeNeeded = SamplesNeeded*2;

if(streams[StreamingHandler].stereo) SizeNeeded*=2;
...
//printf("LastWritePosition     %6i, LastWritePositionBytes %6i\n", streams[StreamingHandler].last_write_pos, LastWritePositionBytes); 
//printf("\n");

if(SamplesNeeded > 0){
SampleBuffer = streams[StreamingHandler].get_data(StreamingHandler, SizeNeeded, &SamplesGotten);
//process_filters(StreamingHandler, &SampleBuffer, &SamplesGotten);
...
spu_memset(streams[StreamingHandler].spu_ram_sch[0] + LastWritePositionBytes, 0, SizeNeeded);
spu_memset(streams[StreamingHandler].spu_ram_sch[1] + LastWritePositionBytes, 0, SizeNeeded);
return(-3);
}

SeparateChannels(SampleBuffer, SizeNeeded, streams[StreamingHandler].stereo);
spu_memload(streams[StreamingHandler].spu_ram_sch[0] + LastWritePositionBytes, (uint8*)StereoSeparationBuffer[0], SizeNeeded);
spu_memload(streams[StreamingHandler].spu_ram_sch[1] + LastWritePositionBytes, (uint8*)StereoSeparationBuffer[1], SizeNeeded);

streams[StreamingHandler].last_write_pos += SamplesNeeded;
if(streams[StreamingHandler].last_write_pos >= SamplesInBuffer) streams[StreamingHandler].last_write_pos -= SamplesInBuffer;
}
...
*DataSizeRetrieved = fs_read(StreamingInputFile, StreamingBuffer, DataSizeRequested);

//printf("DataSizeRequested: (%d)\n", DataSizeRequested);
//printf("DataSizeRetrieved: (%d)\n", *DataSizeRetrieved);

if(*DataSizeRetrieved < DataSizeRequested){
fs_seek(StreamingInputFile, 0x32, SEEK_SET);
fs_read(StreamingInputFile, StreamingBuffer+(*DataSizeRetrieved), DataSizeRequested-(*DataSizeRetrieved));
...
fs_seek(StreamingInputFile, 0, SEEK_SET);
fs_read(StreamingInputFile, &StreamingData.WaveHeaderData, sizeof(WaveHeaderStruct));

if(StreamingData.WaveHeaderData.Format == FormatADPCM) StreamingData.BufferSize = SND_STREAM_BUFFER_MAX/4;
else StreamingData.BufferSize = SND_STREAM_BUFFER_MAX;

//printf("Safe1: (%d)\n", malloc_irq_safe());
//StreamingBuffer = (uint8*)malloc(SND_STREAM_BUFFER_MAX);

StreamingHandler = AllocateStreamingStuff(StreamingDataRetrievalFunction, StreamingData.BufferSize);

StartStreaming(StreamingHandler, StreamingData);

MusicIsPlaying = StreamingActive;
while(MusicIsPlaying == StreamingActive){
thd_sleep(StreamingPollInterval);
CheckForSamples(StreamingHandler, StreamingData);
}
fs_close(StreamingInputFile);
//printf("Safe2: (%d)\n", malloc_irq_safe());
//free(StreamingBuffer);
//StreamingBuffer = NULL;

EndStreaming(StreamingHandler);
MusicIsPlaying = StreamingInActive;

return NULL;
}


void StopStreamingPulseCodeModulationFile(){

if(MusicIsPlaying != StreamingInActive){
while(MusicIsPlaying != StreamingActive){thd_sleep(10);}
MusicIsPlaying = StreamingCleanUp;

thd_join(StreamingThreadIdentifier, NULL);
}
}

void StreamPulseCodeModulationFile(char FileDir[]){
if(MusicIsPlaying == StreamingInActive){
MusicIsPlaying = StreamingLoad;

printf("FileDir: (%s)\n", FileDir);
StreamingInputFile = fs_open(FileDir, O_RDONLY);

StreamingThreadIdentifier = thd_create(0, StreamingThread, NULL);
thd_set_prio(StreamingThreadIdentifier, 15);
}
while(MusicIsPlaying == StreamingLoad){thd_sleep(10);}
