...
int CharHealth, int EnemyHealth, 
int Ticks, uint8* Difficulty
){
...
if(DrawCharX+CharacterTextureSizeDrawn > ScreenSizeX || DrawCharX < 0) return(ComputerAttackSelectJumpForward);
...
else if(EnemyAnimationType == SpecialEffectAnimationIdentifier){
if(CharGroundLevel == 1){
...
if(CharDistanceX > CharWidth || RandomNumber <= 50) return(ComputerAttackSelectMoveForward);
else if(EnemyCrouchPosition != 0) return(ComputerAttackSelectIntoCrouch);
else if(CharDistanceX < (CharWidth/2)) MoveBorderOffset = MoveBorderIdleInterceptorOffset;
else if(EnemyGroundLevel == 0) MoveBorderOffset = MoveBorderIdleAntiJumpOffset;
...
CurrentOffset = fs_seek(CPUMoveArchive, MoveBorderOffset+1, 0);
...
CurrentOffset = fs_seek(CPUMoveArchive, MoveOffset+2, 0);
fs_read(CPUMoveArchive, &AvailabilityCheck, 1);
//printf("Check results in:%d!\n", *AvailabilityCheck);


if(AvailabilityCheck == 0){
MoveBorderMax--;
if(MoveBorderMax < MoveBorderMin){
*AnimationID = -1;
fs_close(CPUMoveArchive);
...
CurrentOffset = fs_seek(CPUMoveArchive, MoveOffset+22, 0);
fs_read(CPUMoveArchive, &AnimationRangeY, 2);

if(AnimationRangeX >= RealCharDistanceX && AnimationRangeY >= CharDistanceY){
*AnimationID = 0;
