...
PaletteFile = fs_open(FileDir, O_RDONLY);

if(PaletteFile == FILEHND_INVALID){fs_close(PaletteFile); return(0);}


FileLength = fs_total(PaletteFile);
PaletteBuffer = (unsigned char*) malloc(FileLength);
fs_read(PaletteFile, PaletteBuffer, FileLength);
fs_close(PaletteFile);
