...
int NewmalFight(int WhichMode, char OriginalCharacterName[][NormalNameSize], char StageName[], char CharIsHuman[], int StartHealth[], int PlayerID[], char PointArray[][SurvivalModeBeatableEnemyArrayLength], uint8* Difficulty){

int Bluff;
int DirtyDeedsDoneDirtCheap;

pvr_ptr_t CharacterTexture[MaxCharsOnScreen];
...
PaletteData CharPalette[MaxCharsOnScreen];
    ...
    int ReleaseButton[MaxCharsOnScreen]; // 0 = Idle condition. Live with it.
    ...
    int CharHitDuration[MaxCharsOnScreen];
    ...
    int SpecialCharAnimation[MaxCharsOnScreen];
    ...
    int SpecialEffectTextureSizeX[MaxCharsOnScreen];
    int SpecialEffectTextureSizeY[MaxCharsOnScreen];
...
//    char DizzyAnimationFlag[MaxCharsOnScreen];
    ...
    int RandomAINumber[MaxCharsOnScreen];
    int JumpLength[MaxCharsOnScreen];
...
//??????????????????
    ...
    LoadCharacterSoundEffects(CharacterSFX, CharacterName);
    ...
    JumpLength[CurrentChar] = (JumpForwardStrength[CurrentChar]*2-4)*CharSpeed[CurrentChar];
    ...
    CharacterTexture[CurrentChar] = pvr_mem_malloc(256*256);
    ...
    CharacterTexture[CurrentChar] = pvr_mem_malloc(128*128*2);
    ...
    FightJustNormalAnimation(FightStage, LeftStageBorder, BackGroundHeaderData, BackGroundData, BackGroundAnimationData, BackGroundTextures, CharacterName, CharacterTexture, AnimationBuffer, WhichAnimationFrame, DrawCharX, PosCharY, CharIsLeft, CharPalette, StageGround, ShadowPositionX, ShadowPositionY, CharacterTextureSizeDrawn, CharacterTextureSizeReal, WhichRound, FightAnimationSoundEffect, FightFightAnimation, &Winner, FightAnimationFlags);
    ...
    CharPalette[CurrentChar].WhichBufferPaletteUsed = CurrentChar; 
    CharacterTextureSizeReal[CurrentChar] = 256;
    pvr_mem_free(CharacterTexture[CurrentChar]);
    CharacterTexture[CurrentChar] = pvr_mem_malloc(256*256);
