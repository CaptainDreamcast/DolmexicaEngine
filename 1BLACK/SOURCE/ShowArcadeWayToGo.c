...
fs_seek(HeaderFile, 0, SEEK_SET);

CurrentChar = CharacterOne;
do{

for(Bluff=0; Bluff<WhoAgainstWhoTextureAmount; Bluff++) fs_read(HeaderFile, &CharTextureData[CurrentChar][Bluff], sizeof(ShowWayToGoTextureDataStruct));
...
fs_read(HeaderFile, EnemyData, sizeof(ShowWayToGoEnemyDataStruct));

CurrentChar = 0;
do{
fs_read(HeaderFile, &AdditionalTextureData[CurrentChar], sizeof(ShowWayToGoTextureDataStruct));
CurrentChar++;
