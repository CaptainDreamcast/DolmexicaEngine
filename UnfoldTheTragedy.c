...
FileSeek(InputFile, CurrentOffset, 0);
FileRead(InputFile, &CurrentOffset, sizeof(uint32));

FileSeek(InputFile, CurrentOffset, 0);

FileRead(InputFile, &WhichStoryBoard, sizeof(uint8));
FileRead(InputFile, BackGroundName, ActualNameSize);
BackGroundName[ActualNameSize] = '\0';

FileRead(InputFile, &EnemyPoolLength, sizeof(uint8));

CurrentOffset = FileTell(InputFile);
//CurrentOffset++;
...
FileSeek(InputFile, CurrentOffset, 0);
FileRead(InputFile, &FinalAICharSelected, sizeof(uint8));
}
FileClose(InputFile);
FileUnlink(FileDir);
