int ArcadeMode(int* WhichPlayerActive){
...
Difficulty = StandardArcadeDifficulty;
...
while(ArcadeLoop == 1){
 ...
 if(ArcadeStage == ActualFightStage){
  Winner = -1;
  Winner = NewmalFight(ArcadeModeIdentifier, CharacterName, BackGroundName, CharIsHuman, CharHealth, PlayerID, PointsArray, &Difficulty);
  //Winner = CharacterOne;
  printf("Winner: (%d)\n", Winner);
