int StoryMode(int* WhichPlayerActive){
...
Difficulty = StandardStoryDifficulty;
...
StoryStage = ActualFightStage;
 ...
 if(StoryStage == ActualFightStage){
  Winner = -1;
  Winner = NewmalFight(StoryModeIdentifier, CharacterName, BackGroundName, CharIsHuman, CharHealth, PlayerID, PointsArray, &Difficulty);
  //Winner = CharacterOne;
  printf("Winner: (%d)\n", Winner);
