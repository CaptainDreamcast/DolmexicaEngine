...
fs_seek(CharacterArchive, 0, 0);
fs_read(CharacterArchive, CharAmount, 2);
...
HeaderFile = fs_open(FileDir, O_RDONLY);
CurrentOffset = fs_seek(HeaderFile, 0, 0);

CurrentChar = CharacterOne;
do{

fs_read(HeaderFile, &FighterSelectHeaderBuffer[CurrentChar], sizeof(FighterSelectHeaderStruct));
...
fs_read(HeaderFile, FighterSelectSpecialBuffer, sizeof(FighterSelectSpecialStruct));
...
fs_seek(CharacterArchive, DesiredOffset, 0);
...
while(CurrentLoadedCharacter < CharacterAmount){
 ...
 fs_read(CharacterArchive, &FighterSelectSmallPortraitBuffer[CurrentLoadedCharacter], sizeof(FighterSelectSmallPortraitStruct));
...
fs_unlink("/ram/SMALLPORTRAIT.pkg");

fs_romdisk_unmount(MountPoint);

fs_close(CharacterArchive);
fs_unlink(CharArchive);
}
...
cdrom_cdda_play(FighterSelectTrack, FighterSelectTrack, 16, CDDA_TRACKS);
...
fs_unlink("/ram/BIGPORTRAIT.pkg");
fs_unlink("/ram/NAME.pkg");
